...
namespace LeagueSandbox.GameServer.Packets.PacketHandlers
{
    public class HandleChatBoxMessage : PacketHandlerBase<ChatMessageRequest>
    {
        ...
        public override bool HandlePacket(int userId, ChatMessageRequest req)
        {
            ...
            if (req.Message.StartsWith(commandStarterCharacter) && _game.Config.ChatCheatsEnabled)
            {
            ...
            var debugMessage = $"{peer.Name} ({peer.Champion.Model}): </font><font color=\"#FFFFFF\">{req.Message}";
            ...
            {
                case ChatType.All:
                    var allTeam = teamChatColor + "[All] " + debugMessage;
                    var allEnemy = enemyChatColor + "[All] " + debugMessage;
                    _game.PacketNotifier.NotifyS2C_SystemMessage(ownTeam, allTeam);
                    _game.PacketNotifier.NotifyS2C_SystemMessage(enemyTeam, allEnemy);
                    return true;
                case ChatType.Team:
                    var dmTeam = teamChatColor + debugMessage;
                    _game.PacketNotifier.NotifyS2C_SystemMessage(ownTeam, dmTeam);
                    return true;
                default:
